# Feature Specification: DeepSeek-Coder-V2-Lite Adaptation

**Feature Branch**: `001-deepseek-coder-v2-lite-adaptation`

**Created**: 2026-05-28

**Status**: Synthetic repairs implemented; live namespace protocol repaired; behavior-limited

**Input**: User description: "Establish baseline and analyze what has to be repaired for `deepseek-ai/DeepSeek-Coder-V2-Lite-Instruct` served through vLLM with `deepseek_v3` tool parser."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Establish Raw Model Baselines (Priority: P1)

As an OpenGate maintainer, I need recorded raw benchmark results for the DeepSeek-Coder-V2-Lite endpoint before any proxy repair claims are made.

**Why this priority**: Raw baselines are the control group for every repair decision and keep later improvements measurable.
...
1. **Given** the DeepSeek endpoint is serving the requested model, **When** the direct smoke benchmark runs, **Then** a report records basic tool-call compatibility metrics.
...
3. **Given** any direct benchmark returns HTTP errors, **When** the report is summarized, **Then** errors are classified separately from tool-call leakage.
...
**Why this priority**: The comparison separates upstream model behavior from OpenGate behavior and prevents accidental overfitting.
...
### User Story 3 - Publish Repair Analysis And Compatibility Status (Priority: P3)

As an OpenGate maintainer, I need a public-friendly model note that explains the DeepSeek setup, baseline scores, repair requirements, and any limitations.
...
- The vLLM endpoint may reject Codex-style roles, native tool history, or tool output items before generation.
- The DeepSeek parser may emit structured tool calls, text tool syntax, both, or neither depending on suite category.
- The chat template may support tool calls for chat completions but produce different Responses-shaped behavior through vLLM.
...
- Some failures may be behavior-only, such as wrong tool choice, planning loops, or task-progress drift; these must not become OpenGate task steering.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: The adaptation MUST record the exact DeepSeek serving facts: model repository, served model name, endpoint, context length, vLLM flags, parser, chat template, and performance mode.
...
- **FR-009**: The adaptation MUST produce a public-friendly DeepSeek model note with setup, commands, report paths, score tables, repair candidates, and compatibility status.
...
- **Model Target**: The served DeepSeek model configuration, including repository, served name, endpoint, context window, parser, template, and launch flags.
...
- **SC-005**: The DeepSeek model note contains enough setup detail for another maintainer to reproduce the endpoint and benchmark sequence.
- **SC-006**: The final compatibility status distinguishes known-good, repair-needed, synthetic-repaired, protocol-blocked, and behavior-limited outcomes.
