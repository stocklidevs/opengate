# Implementation Plan: DeepSeek-Coder-V2-Lite Adaptation

**Branch**: `001-deepseek-coder-v2-lite-adaptation` | **Date**: 2026-05-28 | **Spec**: [spec.md](spec.md)

**Input**: Feature specification from `specs/001-deepseek-coder-v2-lite-adaptation/spec.md`

## Summary

Establish a repeatable compatibility baseline for `DeepSeek-Coder-V2-Lite-Instruct`, implement accepted model-agnostic parser/schema/protocol repairs, and document the remaining live Codex behavior limits. The approach is evidence-first: record serving facts, run direct raw benchmarks, run OpenGate observe/repair comparisons, inspect captures, and only implement repairs for validated protocol, parser, schema, or command-quality failures.
...
**Scale/Scope**: One model adaptation pass for `DeepSeek-Coder-V2-Lite-Instruct`, covering synthetic benchmarks, proxy comparison, failure triage, live smoke readiness, and capture-backed live protocol repair.

## Constitution Check

*GATE: Must pass before Phase 0 research. Re-check after Phase 1 design.*

- **Model-Agnostic Repair**: PASS. The plan allows DeepSeek-specific documentation but only model-agnostic repair code.
...
```text
specs/001-deepseek-coder-v2-lite-adaptation/
|-- spec.md
|-- plan.md
|-- research.md
|-- data-model.md
...
**Structure Decision**: Use the existing single Python package and benchmark harness. This feature started with documentation and evidence, then added source/test changes only after captures proved model-agnostic repair needs.
...
- Treat `deepseek_v3` parser plus the provided chat template as the parser/template pair under validation.
